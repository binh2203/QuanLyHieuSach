...
import service_impl.TaiKhoanServiceImpl;
...
public class Frm_QuenMatKhau extends JFrame implements ActionListener, MouseListener {
	...
	public Frm_QuenMatKhau() {
		...
		lblEmail.setBounds(51, 109, 64, 19);
	...
	private void sendEmail() {
		iNhanVien = new NhanVienServiceImpl();
		try {
			String fromEmail = "conglinhjob2002@gmail.com";
			String password = "rpoajwdekdixtkjk";
			String toEmail = txtEmail.getText();
			String fromName = "Hiệu sách Bình Duy Lĩnh";
			String subject = "Email khôi phục mật khẩu";
			String body = "";
			OTP = String.format("%06d", new Random().nextInt(999999));
			hetHanOTP = new Timestamp(System.currentTimeMillis() + 5 * 60 * 1000);
			body = "Mã OTP: " + OTP + "\n--------------------BDL.BookStore--------------------\nLưu ý: OTP sẽ hết hạn sau 5 phút"+ "\nMọi thắc mắc xin liên hệ: Hiệu sách Bình Duy Lĩnh \nEmail: conglinhjob2002@gmail.com\nPhone: 0966626340";
	...
	private void checkOTP() {
		...
		if (OTP.equals(txtOTP.getText())) {
			...
			if (hetHanOTP.getTime() - currentTime.getTime() < 0) {
	...
	private void doiPass() {
		...
		if (valueXacNhan.trim().equals(valueMoi.trim())) {
			...
			if (iTaiKhoan.doiMatKhau(valueMoi, nv.getMaNV()) == -1)
				return;
			else {
